...
# Required input dataset: 1979_2021_winterSpecEdier_SICbootstrapV31_16pixels.csv"
...
all.ice<-read.csv("output/1979_2021_winterSpecEdier_SICbootstrapV31_16pixels.csv", header=T)
str(all.ice)
head(all.ice)
...
# remove years with incomplete winter data: 1978 (start year, incomplete; data were every-other-day 1978-part way through 1987)
daily_min_sic_core<-subset(daily_min_sic_core, year_winter != 1978 & year_winter != 2021)
table (daily_min_sic_core$year_winter)
...
year<-seq(1979,2020,1) # create dataframe with sequence of years to feed into the function, "year" here is winter year (year in Nov)
...
# 14. Compile variables into dataframe and save ####
sea_ice_vars_1979_2020<-cbind(ws_hi,
                              winter_hi[2],
                              spring_hi[2],
                              ws_lo[2],
                              winter_lo[2],
...
# 15. Save compiled sic variables as csv ####
write.csv(sea_ice_vars_1979_2020, "output/sea_ice_vars_1979_2020.csv")
